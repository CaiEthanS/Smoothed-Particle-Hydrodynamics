function [] = MixedDensities(xMax, yMax, h, N, kappa, mu, rho_0, beta, dt)
% 2D Smoothed Particle Hydrodynamics dam break simulation for a fluid of
% stiffness kappa, viscosity mu, and boundary damping
% beta. Half of the particles have a resting density rho_0, and half have 
% rest density 6*rho_0. The particles are contained in a grid of size xMax 
% by yMax.

% Create video object
% vid = VideoWriter('MixedRhoCase', 'MPEG-4');
% vid.FrameRate = 30;
% vid.Quality = 100;
% open(vid);
...
% Initialize the particles structure (.pos and .vel)
particles(1:N) = struct('pos', [], 'vel', [0, 0], 'force', [0, 0], 'rho', [], 'neighbors', [], 'neighborsDist', []);
sp = 0.09;
k = 1;
while k <= N
    for r = 1:ceil(sqrt(N))
        for c = 1:ceil(sqrt(N))
            particles(k).pos = [r * sp - 0.08, c * sp - 0.08];
            k = k + 1;
        end
    end
end
...
for t = 0:dt:3
    ...
    for k = N/2 + 1:N
        ...
        for n = 1:length(particles(k).neighbors)
            ...
            posDiff = particles(k).pos - particles(j).pos;
            velDiff = particles(k).vel - particles(j).vel;
            Fkj = m*(1 - q)*(15*kappa*(rhoK + rhoJ - rho_1)*(1 - q)*posDiff/q - 40*mu*velDiff)/(pi*(h^4)*rhoJ);
            particles(k).force = particles(k).force + Fkj;
        end
        FExt = [0, -9.8]*rhoK;
        particles(k).force = particles(k).force + FExt;
    ...
    drawnow
    plot(x(1:N/2), y(1:N/2), 'bo', x(N/2 + 1:N), y(N/2 + 1:N), 'ro');
    grid on
    xlim([0,xMax]);
    ylim([0,yMax]);
